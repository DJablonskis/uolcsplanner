...
const TermBox = ({ terms, i, selection }: Props) => {
  ...
  return (
    <div className="bg-orange-100 mx-1 py-2 px-2">
      <div className="flex">
        <span className="ml-2 mr-auto h4 ml-1">Term {i + 1}</span>
        {term.takenCourses.length < 4 && (
          <button
            className="font-bold bg-sky-600 hover:bg-sky-400 hover:text-slate-700 text-slate-100 rounded px-2 py-1  border-sky-900 ml-2"
            onClick={() => selection(i)}
          >
            + Add
          </button>
