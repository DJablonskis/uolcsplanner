...
function ModuleSection({ modules, inc }: Props): JSX.Element {
  let lvl6Available = false;

  let lvl5 = lvl5Access(modules);

  return (
    <div>
      <ModuleAccordion inc={inc} m={modules} lvl={4} />
      {lvl5.available ? (
        <ModuleAccordion inc={inc} m={modules} lvl={5} />
      ) : (
        <div>
      ...
      {lvl6Available ? (
        <ModuleAccordion inc={inc} m={modules} lvl={6} />
      ) : (
        <div>
